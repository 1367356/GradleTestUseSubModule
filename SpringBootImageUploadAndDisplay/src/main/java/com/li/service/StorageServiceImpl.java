...
public class StorageServiceImpl implements StorageService {
	...
	private static final Logger logger = LoggerFactory.getLogger(com.qingwenwei.service.impl.StorageServiceImpl.class);
	...
	public User store(MultipartFile file, String username) { // re-factor needed
		...
		try {
			...
			Path dir = Paths.get(this.staticResourceLocation);  //得到Path 路径
//			Path dir = Paths.get(this.staticResourceLocation + "/" + username);
			if(Files.notExists(dir)) {
				Files.createDirectory(dir);  //创建目录
			}
//			Path avatarLocation = Paths.get(this.staticResourceLocation + "/" + username).resolve(filename);
			Path avatarLocation = Paths.get(this.staticResourceLocation).resolve(filename);
			Files.copy(file.getInputStream(), avatarLocation, StandardCopyOption.REPLACE_EXISTING);  //将接收到的file复制到目录。
			logger.info("Saved file to >> " + avatarLocation);
			
			// update user avatar location
			User user = this.userMapper.findByUsername(username);  //将文件连接存储到该用户数据表下
//			user.setAvatarLocation("avatar/" + username + "/" + filename);
			user.setAvatarLocation("avatar/"  + filename);  //存储
			return user;   //返回用户
		} catch (Exception e) {
