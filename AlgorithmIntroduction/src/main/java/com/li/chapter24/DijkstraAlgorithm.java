...
/**
 * Dijkstra算法求单源最短路径。
 * https://www.cnblogs.com/skywang12345/p/3711512.html?utm_source=tuicool&utm_medium=referral
 * @author liyafei
 *
 */
public class DijkstraAlgorithm {
     ...
     * @author liyafei
     *
     * @param <> 节点中的泛型(代表关键字) 有三个属性，下一节点，关键字，两个节点之间的权重， 权重应该以矩阵的方式存储（也就是一个二维数组）
    ...
    public void shortestPathOfBFS(int vertex){
        ...
        int vertexNum=weis.length;  //定点个数
        int k=weis[vertex].length;  //与vertex相邻的点的个数
        smallWeight=new double[k];
        for (int i = 0; i < smallWeight.length; i++) {
            smallWeight[i]=weights[vertex][i];//将与vertex相邻节点的距离复制出来
        }
        boolean[] weightFound=new boolean[vertexNum];  //默认值为false;
        ...
        for (int i = 0; i < weightFound.length; i++) {
            ...
            for (int j = 0; j < weightFound.length; j++) {  //根据发现的最小值判断，是否有从原点经过最小值点的路径是否比原来的值小的可能性，如果小，就替换
                if(!weightFound[j]){
